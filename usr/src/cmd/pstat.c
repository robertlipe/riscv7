...
#include <sys/text.h>
	...
	for (xp = xtext; xp < &xtext[NTEXT]; xp++, loc+=sizeof(xtext[0])) {
		...
		printf("  %5x", xp->x_caddr);
		printf("%5d", xp->x_size);
		printf("%7x ", kaddr(xp->x_iptr));
...
#include <sys/proc.h>
	...
	for (loc=setup[SPROC].value,pp=xproc; pp<&xproc[NPROC]; pp++,loc+=sizeof(xproc[0])) {
		...
		printf("%5o", pp->p_addr);
		printf("%5o", pp->p_size);
		printf("%7x", kaddr(pp->p_wchan));
		printf("%7x", kaddr(pp->p_link));
		printf("%7x", kaddr(pp->p_textp));
