// largely from https://raw.githubusercontent.com/sipeed/Longan_GD32VF_examples/master/gd32v_lcd/src/lcd/lcd.c
// Correction. That's where this code probably SHOULD have come from. This
// is closer to Longnan's own from https://github.com/sipeed/Longan_GD32VF_examples/blob/master/gd32v_lcd/src/lcd/lcd.c
...
/* LOCAL
void LCD_PutString(const char *p, u16 color)
{
    int x = cursor_x;
    int y = cursor_y;
    while(*p!='\0')
...
LCD_VerticalScrollSetup(unsigned int top_fixed, unsigned bottom_fixed, int scroll_up)
{
 return;
	unsigned scroll_height;
	scroll_height = LCD_H - top_fixed - bottom_fixed;
printf("SH: %d", scroll_height);
	LCD_WR_REG(0x33);
//	LCD_WR_DATA(top_fixed);
//	LCD_WR_DATA(scroll_height);
//	LCD_WR_DATA(bottom_fixed);
	LCD_WR_DATA8(8);
	LCD_WR_DATA8(146);
	LCD_WR_DATA8(8);
#if 0
...
// d8 as above
//	if(USE_HORIZONTAL==2)LCD_WR_DATA8(0x78); // 78 == horiz scroll
	LCD_WR_DATA8(0x6c); 
